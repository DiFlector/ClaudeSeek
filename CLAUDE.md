...
- **Streaming behavior is split by whether tools are declared.** Without `tools` in the request, DeepSeek tokens pass through as Anthropic `text_delta` events in real time. With `tools`, the server buffers the whole response, runs `parseToolCallFromText` (which finds JSON embedded anywhere in the text via `extractJsonCandidates`), and emits either a single `tool_use` block or a single text block as a batch of SSE events at the end. This loses first-token latency for tool-capable requests (which Claude Code always is) but is the only way to handle cases where the model wraps JSON in prose or splits it across many small chunks — an earlier heuristic state machine (undecided/text/tool, `PEEK_THRESHOLD`) couldn't recover once it committed to text mode.
...
- **Tool-call parsing is permissive but post-hoc.** `extractJsonCandidates` finds balanced `{...}` anywhere in the buffered text, including fenced blocks; `parseToolCallFromText` accepts `tool`/`name` + `arguments`/`input` for each. If parsing fails the whole buffer is emitted as a single text block (the raw would-be-tool JSON goes to the client as text — we don't scrub it).
